...
f = open('exported_data.json', encoding="ISO-8859-1")
data = json.load(f)
f.close()
...
results.to_csv("flat_clustering.txt", sep=',', columns=['id', 'cluster'], header=False, index=False, encoding='utf-8')
print("Time taken for storing results of flat clustering: ", time.time() - start_time)

# Define batch size for cosine similarity calculation
batch_size = 1000

# Initialize an empty matrix to store cosine similarity scores
num_samples = len(document_list)
cosine_similarities = np.zeros((num_samples, num_samples))

# Calculate cosine similarity in batches
for i in range(0, num_samples, batch_size):
    # Get the current batch of document vectors
    batch_X = X[i:i + batch_size]
    # Calculate cosine similarity for the current batch
    batch_cosine_similarity = cosine_similarity(batch_X, X)
    # Update the cosine_similarities matrix with the current batch results
    cosine_similarities[i:i + batch_size, :] = batch_cosine_similarity
...
hc_results.to_csv("hierarchical_clustering.txt", sep=',', columns=['id', 'cluster'], header=False, index=False, encoding='utf-8')
